...
{
    public class Update : AbstractFactory.IFactory<Model.ConferenceRoom>
    {
        public bool Parameter(ConferenceRoom _obj)
        {
            return DataAccess.SqlAccess().ExecuteNonQuery(SQL<ConferenceRoom>.Update(_obj, string.Format(" AND ID={0}", _obj.ID))) > 0;
        }
