...
{
    public class Delete : AbstractFactory.IFactory<Model.ConferenceRoom>
    {
        public bool Parameter(ConferenceRoom _obj)
        {
            return DataAccess.SqlAccess().ExecuteNonQuery(string.Format("DELETE FROM ConferenceRoom WHERE ID={0}",_obj.ID)) > 0;
        }
