<%@ Page Title="" Language="C#" MasterPageFile="~/Master/SubMaster/SubEnterpriseManageMasterPage.master" AutoEventWireup="true" CodeFile="Edit.aspx.cs" Inherits="EnterpriseManage_Financing_Edit" %>
...
<asp:Content ID="Content5" ContentPlaceHolderID="phPageContent" runat="Server">
    <div class="bottom">
        <ul>
            ...
            <li title="请输入联系电话">
                <span>联系电话</span>
                <input type="tel" name="" id="Mobile" value="<%=info.Mobile.Trim() %>" placeholder="请输入电话" />
                <b class="required">*</b>
            </li>
            <li title="请输入融资金额，输入数字即可">
                <span>融资规模</span>
                <input type="tel" name="" id="Money" value="<%=info.Money %>" placeholder="请输入融资金额" />万元
                <b class="required">*</b>
            </li>
            <li title="请输入出让的股权百分比">
                <span>出让股权</span>
                <input type="tel" name="" id="Stock" value="<%=info.Stock.Trim().Replace("%","") %>" placeholder="请输入股权百分比，范围1-100" />%
			    <b class="required">*</b>
            </li>
            <li>
                <span>预期回报</span>
            ...
            <li style="width: 100%;" title="请输入融资的用途等" class="detail">
                ...
                <textarea id="Detail"><%=info.Detail.Trim() %></textarea>
                <b class="required">*</b>
            </li>
            <li style="width: 100%;">
...
<asp:Content ID="Content6" ContentPlaceHolderID="phScript" runat="Server">
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $("#Stage").val("<%=info.Stage.Trim()%>");
            $("#Profit").val("<%=info.Profit.Trim()%>");
            ...
            var postFunc = function (e) {
                ...
                var data = { "EnterpriseId": "<%=((Master_SubMaster_SubEnterpriseManageMasterPage)(this.Master)).user.EnterpriseId%>", "UserId": "<%=((Master_SubMaster_SubEnterpriseManageMasterPage)(this.Master)).user.ID%>", "Stage": escape($.trim($("#Stage").val())), "Mobile": escape($.trim($("#Mobile").val())), "Money": escape($.trim($("#Money").val())), "Stock": escape($.trim($("#Stock").val().replace("%", "")) + "%"), "Profit": escape($.trim($("#Profit").val())), "Detail": escape($.trim($("#Detail").val())), "State": 0, "ID": <%=info.ID%> };
                $.post("../../Handler/UpdateFinancingHandler.ashx", data, function (res) {
