<%@ Page Title="" Language="C#" MasterPageFile="~/Master/SubMaster/SubEnterpriseManageMasterPage.master" AutoEventWireup="true" CodeFile="Edit.aspx.cs" Inherits="EnterpriseManage_User_Edit" %>
...
<asp:Content ID="Content5" ContentPlaceHolderID="phPageContent" runat="Server">
    <div class="bottom">
        <ul>
            <li title="必填">
                <span>手机号码</span>
                <input type="text" name="" id="Mobile" value="<%=user.Mobile.Trim() %>" placeholder="请输入登录账号" />
                <b class="required">*</b>
            </li>
            <li title="必填">
                <span>真实姓名</span>
                <input type="text" name="" id="Name" value="<%=user.Name.Trim() %>" placeholder="请输入真实姓名" />
                <b class="required">*</b>
            </li>
            <li title="必填">
                <span>密　　码</span>
                <input type="password" name="" id="Password" value="<%=user.Password.Trim() %>" placeholder="请输入密码" />
                <b class="required">*</b>
            </li>
            <li title="不一致">
                <span>确认密码</span>
                <input type="password" name="" id="cmPassword" value="<%=user.Password.Trim() %>" placeholder="再次输入密码" />
                <b class="required">*</b>
            </li>
            <li title="必填">
                <span>公司职位</span>
                <input type="text" name="" id="Post" value="<%=user.Post.Trim() %>" placeholder="请输入公司职位" />
                <b class="required">*</b>
            </li>
...
<asp:Content ID="Content6" ContentPlaceHolderID="phScript" runat="Server">
    <script type="text/javascript">
        $(function () {
            ...
            var permission = '<%=user.Permission.Trim()%>';
            if (permission == "all") {
                ...
                for (var p in permission)
                {
                    for (var v in p)
                    {
                        console.log(permission[p][v]);
                        $("input[type='checkbox'][value='" + permission[p][v] + "']").prop("checked", true);
                    }
                }
            }
            ...
            var postFunc = function (e) {
                ...
                var data = { Mobile: $.trim($("#Mobile").val()), Password: $.trim($("#Password").val()), Name: escape($.trim($("#Name").val())), Permission: Permission, Post: escape($.trim($("#Post").val())), "ID": "<%=user.ID%>" };
                $.post("../../Handler/UpdateEntUserHandler.ashx", data, function (res) {
