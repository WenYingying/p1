<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Add.aspx.cs" Inherits="_Management_Information_Add" %>
...
<body>
    <div class="iframe_right">
        <div class="title">
            <h2>发布<%=Common.GetInstance.GetInfoType(Request.QueryString["Type"]) %></h2>
        </div>
        <div class="enter_info">
            <ul>
                <li>
                    <span>标题</span>
                    <input type="text" name="" id="Title" value="" placeholder="请输入信息标题" />
                </li>
                <li>
                    <span>封面</span>
                    <div class="banner_img">
                        ...
                        <input type="hidden" name="" id="CoverPic" value="" />
                    </div>
                    <div class="hint_box">
                ...
                <li>
                    <span>内容</span>
                    <textarea name="" rows="" cols="" id="Content"></textarea>
                </li>
                <li>
                    <span></span>
                    <div>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            var postFunc = function (e) {
                ...
                var data = { "Title": escape($.trim($("#Title").val())), "Type": "<%=Request.QueryString["Type"]%>", "Tags": "", "Content": escape($.trim($("#Content").val())), "CoverPic": escape($.trim($("#CoverPic").val())), "AdminId": "<%=Request.Cookies["Admin"]["ID"]%>","State":1 };
                $.post("../../Handler/PostInformationHandler.ashx", data, function (res) {
                    var json = JSON.parse(res);
                    if (json.Result) {
                        alert("发布成功！");
                        location.href = "Default.aspx?Type=<%=Request.QueryString["Type"]%>";
                        return;
                    }
